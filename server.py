...
class RasaCoreServer(object):
    ...
    @app.route("/conversations/<sender_id>/getEntity",
               methods=['GET', 'POST', 'OPTIONS'])
    @check_cors
    @requires_auth
    @ensure_loaded_agent
    def getEntity(self, request, sender_id):
        request.setHeader('Content-Type', 'application/json')
        request_params = request_parameters(request)
        if 'entity' in request_params:
            entity = request_params.pop('entity')
        else:
            request.setResponseCode(400)
            return json.dumps({"error": "Invalid parse parameter specified"})
        try:
            path = 'dialogflow/entities/'+ entity + '_entries_en.json'
